...
if import_error != "":
    print "Need some libs !" + import_error
    raise SystemExit

from intelwiz.core.constants import INTELWIZ_PATH
from intelwiz import VERSION


class install():
    def quitDialog(self, widget, event=None):
        if widget == self.btn_modules:
            self.msgbox("To use 'intelwiz', launch it with this command")
            exit()
        else:
            if self.yesnoDialog("Do you really want to quit\nthe installation wizard?"):
                exit()
            else:
                return 1
    ...
    def msgbox(self, message, type_msg=0):
        about = gtk.MessageDialog(self.fenetre,
                                  ...
                                  gtk.BUTTONS_OK,
                                  message)
        about.run()  # Show dialog box
        about.destroy()  # Destruct dialog box
                    ...
                    for x in installType:
                        ...
                        if x == 'easy_install':
                            ...
                            while True:
                                ...
                                if err:
                                    print '@', err  # TODO color ? red ?
                                out = self.eiProcess.stdout.readline().rstrip('\n')
                                if out:
                                    print '#', out  # TODO color ? green ?
                                    if "No local packages or download links found" in out:
                                        print "=> Package not found with easy_install"  # TODO error
    ...
    def __init__(self):
        ...
        self.fenetre.set_title("Intelligence Wizard - Installation Wizard") # Titre de la fenêtre
        #self.fenetre.set_decorated(False) # Cacher les contours de la fenêtre
        self.fenetre.set_icon_from_file("%s/images/icone.png" % INTELWIZ_PATH) # Spécifier une icône
        ...
        self.img_a_propos.set_from_file("%s/images/a_propos.png" % INTELWIZ_PATH)
        ...
        json_data=open('%s/core/modules.json' % INTELWIZ_PATH)
        ...
        self.label_entree_modules = gtk.Label("MIT License")
